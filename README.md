...
| <a name="input_sandbox_allow_public_egress"></a> [sandbox\_allow\_public\_egress](#input\_sandbox\_allow\_public\_egress) | Allow sandbox pods to reach the public internet (public DNS + HTTPS, private ranges excluded) and the Envoy egress proxy. Default false: sandbox egress is deny-all except kube-dns and sandbox\_egress\_allow, so untrusted agent code has no route to exfiltrate data (e.g. model weights). Any allowed public destination, even behind a host allowlist, is an exfiltration channel; enable only if you accept that. | `bool` | `false` | no |
| <a name="input_sandbox_blocked_egress_cidrs"></a> [sandbox\_blocked\_egress\_cidrs](#input\_sandbox\_blocked\_egress\_cidrs) | Only used when sandbox\_allow\_public\_egress = true. Extra CIDRs sandbox pods must never reach, on top of RFC1918, 100.64.0.0/10 (CGNAT, used by EKS custom networking pod CIDRs), 169.254.0.0/16 (link-local/IMDS), var.vpc\_cidr, the subnet CIDRs, and the cluster service CIDR. Add secondary VPC CIDRs, peered VPCs, or on-prem ranges here. | `list(string)` | `[]` | no |
